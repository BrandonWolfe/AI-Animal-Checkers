 ...
 * @brief This class comprises the game board setup, and play (both AI and human)
...
public class MainActivity extends Activity {
	...
	private boolean AIturnOne;
	   ...
	   //Maintains the ids of the buttons in a one-to-one relationship with an index value  
	   ...
	   //image ids
	   int drawableIds[]={R.drawable.grasstile, R.drawable.whitemouse,R.drawable.grasstile,R.drawable.den,R.drawable.grasstile,R.drawable.whiteelephant,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.whitetiger,R.drawable.grasstile,R.drawable.whitewolf,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.grasstile,R.drawable.blackwolf,R.drawable.grasstile,R.drawable.blacktiger,R.drawable.grasstile,R.drawable.grasstile,
			   R.drawable.grasstile,R.drawable.blackelephant,R.drawable.grasstile,R.drawable.den,R.drawable.grasstile,R.drawable.blackmouse,R.drawable.grasstile,};
 ...
 * extensibility in terms of not needin to tie the opening move to minimax output.
...
/**
 * setupBoard()
 * @brief Used once, upon startup. Draws a new board to screen and handles the logic behind button clicks.
 * First button selected represents piece moving from, second button in the sequence represents selected piece move to spot.
 * @pre imagebutton ids, drawable ids and player pieces must be defined first
 * @post 7x9 grid of image buttons is displayed to screen. Game is ready to play. 
 */
...
/**
 * makeMove()
 * @brief Performs the array swaps of both piece ids and drawable ids, corrsponding to a single move.
 * @post Human players move is updated. If playing against AI, there move is updated as well. (2ply)
...
public boolean isValid(int fromID, int toID){
	...
	case(R.id.a6):
	{
		if((going_to == (coming_from + 1)) || (going_to == (coming_from + 7)) || (going_to == (coming_from - 1))){
	...
	case(R.id.h1):
	{
		if((going_to == (coming_from + 1)) || (going_to == (coming_from + 7)) || (going_to == (coming_from - 7))){
			if(!isOccupied(going_to, coming_from))
				retVal = true;
				else
				retVal=false;
	...
	case(R.id.h7):
	{
		if((going_to == (coming_from - 7)) || (going_to == (coming_from + 7)) || (going_to == (coming_from - 1))){
			if(!isOccupied(going_to, coming_from))
				retVal = true;
				else
			retVal=false;
	...
	{
		if((going_to == (coming_from - 1)) || (going_to == (coming_from - 7)) || (going_to == (coming_from + 7)) || (going_to == (coming_from + 1))){	
			if(!isOccupied(going_to, coming_from))
				retVal = true;
				else
				retVal=false;
...
public void takePawn(int to){
	...
	//turns "to" location containing valid "take-able" opponent pawn into a grass tile
	...
	if(myturn){//player one's the current player
		...
		//deletes the piece from the array and decrements the opposing players total pawn count
		player2pieces[to] = 0;
		player2pawnCount--;
 ...
 * @param depth, desired number of ply (8 is the upper limit on Android device)
 ...
 * @return a measurement of the board indicating fitness of a single move. Higher the better, 1000 max.
 */
public int SEF(AIboard gameBoard, boolean AI_p1)
{
	...
	for(int x = 0; x < 4; ++x)		
	{
		...
		//if there is a valid piece to take at a valid spot then this is likely preferable to forward marching 
